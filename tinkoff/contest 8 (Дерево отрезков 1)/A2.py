...
class SimpleSegmentTree:
    ...
    def __init__(self, A):
        self.n = len(A)
        self.A = A
        self.neutral = 0 # нейтральный элемент нашей операции
        self.ST = [self.neutral for _ in range(4 * self.n)] # само дерево отрезков, можно лучше, но зачем?
        self.build(1, 0, self.n - 1) # внутреннее построение
